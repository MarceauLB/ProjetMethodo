...
res_times <- matrix(0, ncol = 30, nrow=10, byrow = TRUE)
...
for (nb_feature_generated in seq(100, 1000, 100)){
  ...
  res_times[nb_feature_generated/100,] <- list_time
}

write.csv(res_times,file="ckta_time_features.csv",row.names = FALSE)

rm <- rowMeans(res_times)
